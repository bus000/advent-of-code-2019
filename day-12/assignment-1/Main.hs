...
handleInput = print . energy . simulateSteps 1000

energy :: Bodies -> Int
energy = sum . map bodyEnergy
  where
    bodyEnergy (Body (x, y, z) (dx, dy, dz)) =
        (abs x + abs y + abs z) * (abs dx + abs dy + abs dz)

simulateSteps :: Int -> Bodies -> Bodies
simulateSteps n = head . drop n . iterate step
